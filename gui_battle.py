...
# ==========================================================
# IMPORTAÇÃO DA LÓGICA ORIGINAL (Assumindo que estão disponíveis)
# ==========================================================
# from heroi import Heroi # A classe do seu herói
# from personagem import Personagem # A classe base para o chefe
# from batalha import Batalha # A lógica de dano e combate
from game_assets import BOSSES_DATA # Dados do novo arquivo

# ==========================================================
# MOCKUP (Para rodar o código sem o seu ecossistema completo)
# REMOVA ESTES MOCKS SE ESTIVER IMPORTANDO AS CLASSES ORIGINAIS
# ==========================================================
class MockPersonagem:
    def __init__(self, nome, vida, ataque, defesa, estamina, carga=0):
        ...
        self.trufa = [{'nome': 'Trufa de morango', 'qnt': 3}] # Simplificado

class MockBatalha:
    # Apenas métodos simulados para o GUI
    usadas_em_batalha = {}
    ...
    def defender(self, personagem):
        personagem.defesa += 10 # Simulação
    ...
    def usar_trufa(self, personagem):
        personagem.vida = min(personagem.vidabase, personagem.vida + 30) # Simulação Trufa de Morango
        
# Use as classes Mock para demonstração:
...
def desenhar_texto(surface, text, font, color, x, y, center=False):
    """Desenha um texto na superfície."""
...
def desenhar_barra_status(surface, x, y, width, height, current, maximum, color_fill, label):
    """Desenha uma barra de vida/estamina/carga."""
    # Fundo da barra
...
class BattleScene:
    def __init__(self, player, boss_key):
        ...
        # 1. PERSONAGENS
        self.player = player
        # Cria a instância do chefe (usando Personagem original ou Mock)
        self.enemy = Personagem(
            boss_data['nome'],
            *boss_data['stats'] # (Vida, Ataque, Defesa, Estamina, Carga)
        )
        self.batalha_logic = Batalha() # Instância da sua lógica de batalha

        # 2. ESTADOS
        self.state = 'DIALOGUE' # DIALOGUE, PLAYER_TURN, ENEMY_TURN, BATTLE_END
        self.current_dialogue = boss_data['dialogue']
        self.dialogue_index = 0
        self.log_messages = [] # Mensagens curtas de combate (dano, crítico, etc.)
    ...
    def load_assets(self, data):
        """Carrega e dimensiona imagens."""
        try:
            # Tente carregar o fundo
        ...
        # Sprites
        # ATENÇÃO: Use `os.path.join` para caminhos corretos ou caminhos relativos
        try:
            ...
            self.sprite_player.fill(AZUL) # Placeholder azul
            self.sprite_enemy = pygame.Surface((200, 250), SRCALPHA)
            self.sprite_enemy.fill(VERMELHO) # Placeholder vermelho


    def setup_ui(self):
        """Configura os botões de ação."""
        w, h = 180, 40
        x_start, y_start = 590, LARGURA - 170 # Área do menu de ações
        padding = 10
    ...
    def handle_input(self, event):
        """Gerencia os cliques do mouse/teclas."""
        if self.state == 'DIALOGUE':
            if event.type == MOUSEBUTTONDOWN or (event.type == KEYDOWN and event.key == K_SPACE):
                ...
                if self.dialogue_index >= len(self.current_dialogue):
                    self.state = 'PLAYER_TURN' # Termina o diálogo, começa a batalha
                return
            ...
            for button in self.action_buttons:
                if button.is_clicked(event):
                    ...
                    # Limpa log anterior e executa a ação
                    self.log_messages = []
                    
                    # Chama o método de batalha e passa os personagens
                    # Nota: O método 'atacar' do seu código original ainda printa no console.
                    # Você precisaria modificar 'batalha.py' para redirecionar esses prints 
                    # para uma lista ou função global para que apareçam na caixa de diálogo da GUI.
                    ...
                    # Após a ação do jogador, é a vez do inimigo
                    self.state = 'ENEMY_TURN'
                    return

    def update(self):
        """Lógica de atualização (turnos, checagem de vitória)."""
        if self.state == 'ENEMY_TURN':
            
            # Checa se a batalha terminou após a ação do jogador
            if self.enemy.vida <= 0:
                self.state = 'BATTLE_END'
                self.log_messages.append(f"{self.player.nome} Venceu!")
                return
            
            # Simulação de Ação do Inimigo (Baseado em combate.py dados_idimigo)
            # Simplificado para Ataque Leve para o propósito da GUI
            pygame.time.wait(1000) # Pequena pausa para simular o turno do inimigo
    ...
    def draw(self, surface):
        ...
        # 4. Caixa de Diálogo/Log de Ações
        ...
        if self.state == 'DIALOGUE':
            if self.dialogue_index < len(self.current_dialogue):
                ...
                # Quebra de linha simples para o diálogo
                words = text.split(' ')
                line_to_draw = ""
                y_offset = 0
                for word in words:
        ...
        else: # Exibe Log de Combate
            for i, msg in enumerate(self.log_messages[-3:]): # Últimas 3 mensagens
        ...
        # 5. Menu de Ações (Somente no turno do jogador)
        if self.state == 'PLAYER_TURN':
            for button in self.action_buttons:
                button.draw(surface)
            ...
            message = self.log_messages[-1] # Venceu ou Derrota
...
# ==========================================================
def main_game_loop():
    # Instancia o herói com alguns stats base (Ajuste conforme sua classe Heroi)
    player = Heroi(nome="Aluno", vida=120, ataque=25, defesa=10, estamina=5, carga=0)
    
    # Inicia a cena de batalha (usando o primeiro chefe como exemplo)
    try:
    ...
    while rodando:
        for event in pygame.event.get():
            ...
            # Gerenciar clique/input para a cena atual
            if current_scene:
...
if __name__ == '__main__':
    # NOTE: Certifique-se de que Heroi, Personagem, Batalha e o seu util.py
    # estejam importáveis ou substitua-os pelos MOCKUPs acima para teste.
    main_game_loop()